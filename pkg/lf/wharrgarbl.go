...
func Wharrgarbl(in []byte, difficulty uint32, minMemorySize uint) (out [20]byte, iterations uint64) {
	...
	iterations = uint64(C.ZTLF_Wharrgarbl(unsafe.Pointer(&out), unsafe.Pointer(&in[0]), 64, _Ctype_uint(difficulty), wharrgarblMemory, _Ctype_ulong(wharrgarblMemorySize), _Ctype_uint(runtime.NumCPU())))
