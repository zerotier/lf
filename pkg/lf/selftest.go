...
func TestCore(out io.Writer) bool {
	...
	if hex.EncodeToString(op384.Bytes()) != "0a6decbefeaeaeee286ce3acd26aa4a62e9ce05fa2c8f65dd0d023995d6aa7255e84b570fa79e9c9dfb325d8e0344702b9" {
		fmt.Fprintf(out, "FAILED %x\n", op384.Bytes())
		return false
	}
	fmt.Fprint(out, "ed25519 ")
	o25519, _ := NewOwnerFromSeed(OwnerTypeEd25519, []byte("lol"))
	if hex.EncodeToString(o25519.Bytes()) != "0549caaa0d533dbfe6071be32ab0cf3d6cbb8b53394216573e9fa2db5df79e15" {
		fmt.Fprintf(out, "FAILED %x\n", o25519.Bytes())
		return false
	}
	fmt.Fprintf(out, "OK\n")
...
func TestDatabase(testBasePath string, out io.Writer) bool {
	...
	for dbi2 := 0; dbi2 < testDatabaseInstances; dbi2++ {
		...
		go func() {
			...
			rb := make([]byte, 0, 4096)
			for ri := 0; ri < testDatabaseRecords; ri++ {
				err = dbs[dbi].query(0, 9223372036854775807, [][2][]byte{{selectorKeys[ri], selectorKeys[ri]}}, func(ts, weightL, weightH, doff, dlen uint64, localReputation int, id *[32]byte, owner []byte) bool {
	...
	for dbi2 := 0; dbi2 < testDatabaseInstances; dbi2++ {
		...
		go func() {
			...
			for oi := 0; oi < testDatabaseOwners; oi++ {
				...
				err = dbs[dbi].query(0, 9223372036854775807, [][2][]byte{{sk0, sk1}}, func(ts, weightL, weightH, doff, dlen uint64, localReputation int, id *[32]byte, owner []byte) bool {
