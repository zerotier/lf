...
int ZTLF_DB_Open(struct ZTLF_DB *db,const char *path)
{
	...
	/* Add a new record. */
	if ((e = sqlite3_prepare_v2(db->dbc,"INSERT INTO record (doff,dlen,goff,ts,exp,score,link_count,id,owner,hash,new_owner,sel0,sel1) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",-1,&db->sAddRecord,NULL)) != SQLITE_OK)
		goto exit_with_error;
	...
	if ((e = sqlite3_prepare_v2(db->dbc,"SELECT (doff + dlen) ORDER BY doff DESC LIMIT 1",-1,&db->sGetDataSize,NULL)) != SQLITE_OK)
		goto exit_with_error;
	...
	if ((e = sqlite3_prepare_v2(db->dbc,"SELECT r.doff,r.dlen,r.goff,r.ts,r.exp,r.id,r.owner,r.new_owner FROM record AS r WHERE (SELECT COUNT(1) FROM dangling_link AS dl WHERE dl.linking_record_goff = r.goff) = 0 ORDER BY r.ts ASC",-1,&(db->sGetMatching[0]),NULL)) != SQLITE_OK) goto exit_with_error;
	if ((e = sqlite3_prepare_v2(db->dbc,"SELECT r.doff,r.dlen,r.goff,r.ts,r.exp,r.id,r.owner,r.new_owner FROM record AS r WHERE r.sel1 = ? AND (SELECT COUNT(1) FROM dangling_link AS dl WHERE dl.linking_record_goff = r.goff) = 0 ORDER BY r.ts ASC",-1,&(db->sGetMatching[1]),NULL)) != SQLITE_OK) goto exit_with_error;
	if ((e = sqlite3_prepare_v2(db->dbc,"SELECT r.doff,r.dlen,r.goff,r.ts,r.exp,r.id,r.owner,r.new_owner FROM record AS r WHERE r.sel0 = ? AND (SELECT COUNT(1) FROM dangling_link AS dl WHERE dl.linking_record_goff = r.goff) = 0 ORDER BY r.ts ASC",-1,&(db->sGetMatching[2]),NULL)) != SQLITE_OK) goto exit_with_error;
	if ((e = sqlite3_prepare_v2(db->dbc,"SELECT r.doff,r.dlen,r.goff,r.ts,r.exp,r.id,r.owner,r.new_owner FROM record AS r WHERE r.sel0 = ? AND r.sel1 = ? (SELECT COUNT(1) FROM dangling_link AS dl WHERE dl.linking_record_goff = r.goff) = 0 ORDER BY r.ts ASC",-1,&(db->sGetMatching[3]),NULL)) != SQLITE_OK) goto exit_with_error;
	if ((e = sqlite3_prepare_v2(db->dbc,"SELECT r.doff,r.dlen,r.goff,r.ts,r.exp,r.id,r.owner,r.new_owner FROM record AS r WHERE r.owner = ? AND (SELECT COUNT(1) FROM dangling_link AS dl WHERE dl.linking_record_goff = r.goff) = 0 ORDER BY r.ts ASC",-1,&(db->sGetMatching[4]),NULL)) != SQLITE_OK) goto exit_with_error;
	if ((e = sqlite3_prepare_v2(db->dbc,"SELECT r.doff,r.dlen,r.goff,r.ts,r.exp,r.id,r.owner,r.new_owner FROM record AS r WHERE r.owner = ? AND r.sel1 = ? AND (SELECT COUNT(1) FROM dangling_link AS dl WHERE dl.linking_record_goff = r.goff) = 0 ORDER BY r.ts ASC",-1,&(db->sGetMatching[5]),NULL)) != SQLITE_OK) goto exit_with_error;
	if ((e = sqlite3_prepare_v2(db->dbc,"SELECT r.doff,r.dlen,r.goff,r.ts,r.exp,r.id,r.owner,r.new_owner FROM record AS r WHERE r.owner = ? AND r.sel0 = ? AND (SELECT COUNT(1) FROM dangling_link AS dl WHERE dl.linking_record_goff = r.goff) = 0 ORDER BY r.ts ASC",-1,&(db->sGetMatching[6]),NULL)) != SQLITE_OK) goto exit_with_error;
	if ((e = sqlite3_prepare_v2(db->dbc,"SELECT r.doff,r.dlen,r.goff,r.ts,r.exp,r.id,r.owner,r.new_owner FROM record AS r WHERE r.owner = ? AND r.sel0 = ? AND r.sel1 = ? AND (SELECT COUNT(1) FROM dangling_link AS dl WHERE dl.linking_record_goff = r.goff) = 0 ORDER BY r.ts ASC",-1,&(db->sGetMatching[7]),NULL)) != SQLITE_OK) goto exit_with_error;
	if ((e = sqlite3_prepare_v2(db->dbc,"SELECT r.doff,r.dlen,r.goff,r.ts,r.exp,r.id,r.owner,r.new_owner FROM record AS r WHERE r.id = ? AND (SELECT COUNT(1) FROM dangling_link AS dl WHERE dl.linking_record_goff = r.goff) = 0 ORDER BY r.ts ASC",-1,&(db->sGetMatching[8]),NULL)) != SQLITE_OK) goto exit_with_error;
	if ((e = sqlite3_prepare_v2(db->dbc,"SELECT r.doff,r.dlen,r.goff,r.ts,r.exp,r.id,r.owner,r.new_owner FROM record AS r WHERE r.id = ? AND r.sel1 = ? AND (SELECT COUNT(1) FROM dangling_link AS dl WHERE dl.linking_record_goff = r.goff) = 0 ORDER BY r.ts ASC",-1,&(db->sGetMatching[9]),NULL)) != SQLITE_OK) goto exit_with_error;
	if ((e = sqlite3_prepare_v2(db->dbc,"SELECT r.doff,r.dlen,r.goff,r.ts,r.exp,r.id,r.owner,r.new_owner FROM record AS r WHERE r.id = ? AND r.sel0 = ? AND (SELECT COUNT(1) FROM dangling_link AS dl WHERE dl.linking_record_goff = r.goff) = 0 ORDER BY r.ts ASC",-1,&(db->sGetMatching[10]),NULL)) != SQLITE_OK) goto exit_with_error;
	if ((e = sqlite3_prepare_v2(db->dbc,"SELECT r.doff,r.dlen,r.goff,r.ts,r.exp,r.id,r.owner,r.new_owner FROM record AS r WHERE r.id = ? AND r.sel0 = ? AND r.sel1 = ? AND (SELECT COUNT(1) FROM dangling_link AS dl WHERE dl.linking_record_goff = r.goff) = 0 ORDER BY r.ts ASC",-1,&(db->sGetMatching[11]),NULL)) != SQLITE_OK) goto exit_with_error;
	if ((e = sqlite3_prepare_v2(db->dbc,"SELECT r.doff,r.dlen,r.goff,r.ts,r.exp,r.id,r.owner,r.new_owner FROM record AS r WHERE r.id = ? AND r.owner = ? AND (SELECT COUNT(1) FROM dangling_link AS dl WHERE dl.linking_record_goff = r.goff) = 0 ORDER BY r.ts ASC",-1,&(db->sGetMatching[12]),NULL)) != SQLITE_OK) goto exit_with_error;
	if ((e = sqlite3_prepare_v2(db->dbc,"SELECT r.doff,r.dlen,r.goff,r.ts,r.exp,r.id,r.owner,r.new_owner FROM record AS r WHERE r.id = ? AND r.owner = ? AND r.sel1 = ? AND (SELECT COUNT(1) FROM dangling_link AS dl WHERE dl.linking_record_goff = r.goff) = 0 ORDER BY r.ts ASC",-1,&(db->sGetMatching[13]),NULL)) != SQLITE_OK) goto exit_with_error;
	if ((e = sqlite3_prepare_v2(db->dbc,"SELECT r.doff,r.dlen,r.goff,r.ts,r.exp,r.id,r.owner,r.new_owner FROM record AS r WHERE r.id = ? AND r.owner = ? AND r.sel0 = ? AND (SELECT COUNT(1) FROM dangling_link AS dl WHERE dl.linking_record_goff = r.goff) = 0 ORDER BY r.ts ASC",-1,&(db->sGetMatching[14]),NULL)) != SQLITE_OK) goto exit_with_error;
	if ((e = sqlite3_prepare_v2(db->dbc,"SELECT r.doff,r.dlen,r.goff,r.ts,r.exp,r.id,r.owner,r.new_owner FROM record AS r WHERE r.id = ? AND r.owner = ? AND r.sel0 = ? AND r.sel1 = ? AND (SELECT COUNT(1) FROM dangling_link AS dl WHERE dl.linking_record_goff = r.goff) = 0 ORDER BY r.ts ASC",-1,&(db->sGetMatching[15]),NULL)) != SQLITE_OK) goto exit_with_error;
...
void ZTLF_DB_Close(struct ZTLF_DB *db)
{
	...
	if (db->dbc) {
		...
		for(int i=0;i<16;++i) {
			if (db->sGetMatching[i])
				sqlite3_finalize(db->sGetMatching[i]);
		}
...
{
	sqlite3_stmt *const s = db->sGetMatching[((id) ? 8 : 0) | ((owner) ? 4 : 0) | ((sel0) ? 2 : 0) | ((sel1) ? 1 : 0)];

	pthread_mutex_lock(&db->dbLock);

	sqlite3_reset(s);
	int col = 1;
	if (id) sqlite3_bind_blob(s,col++,id,32,SQLITE_STATIC);
	if (owner) sqlite3_bind_blob(s,col++,owner,32,SQLITE_STATIC);
	if (sel0) sqlite3_bind_blob(s,col++,sel0,32,SQLITE_STATIC);
	if (sel1) sqlite3_bind_blob(s,col++,sel1,32,SQLITE_STATIC);

	while (sqlite3_step(s) == SQLITE_ROW) {
		const struct ZTLF_DB_GraphNode *gn = ZTLF_MappedFile_TryGet(&db->gf,(uintptr_t)sqlite3_column_int64(s,2),ZTLF_DB_MAX_GRAPH_NODE_SIZE);
		if (gn) {
			if (f(sqlite3_column_int64(s,0),sqlite3_column_int64(s,1),(uint64_t)sqlite3_column_int64(s,3),(uint64_t)sqlite3_column_int64(s,4),(void *)sqlite3_column_blob(s,5),(void *)sqlite3_column_blob(s,6),(void *)sqlite3_column_blob(s,7),ZTLF_get64_le(gn->weightL),(uint64_t)ZTLF_get16_le(gn->weightH),arg))
				break;
		}
	}
...
uint64_t ZTLF_DB_CRC64(struct ZTLF_DB *db)
{
	...
	while (sqlite3_step(db->sGetAllRecords) == SQLITE_ROW) {
		...
		if (gn) {
			uint64_t wbig = ZTLF_htonll((uint64_t)gn->weightL);
			crc = _ZTLF_CRC64(crc,(const uint8_t *)&wbig,sizeof(wbig));
			wbig = ZTLF_htonll((uint64_t)gn->weightH);
			crc = _ZTLF_CRC64(crc,(const uint8_t *)&wbig,sizeof(wbig));
