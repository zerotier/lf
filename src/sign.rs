...
impl KeyPair {
    pub fn generate() -> KeyPair {
        let ctx: &mut BigNumContext = unsafe { &mut crate::BIGNUM_CONTEXT };
        ...
        let p384_pub = p384.public_key().to_bytes(group.as_ref(), openssl::ec::PointConversionForm::COMPRESSED, ctx).unwrap();
    ...
    pub fn sign(&self, msg: &[u8]) -> Option<[u8; P384_SIGNATURE_SIZE]> {
        ...
        let p384_pub = EcPoint::from_bytes(group.as_ref(), &self.0[1..P384_PUBLIC_KEY_SIZE], ctx);
        if p384_pub.is_err() {
            return None;
        }
        ...
        let mut p384sr = p384s.r().to_vec();
        let mut p384ss = p384s.s().to_vec();
...
pub fn verify(msg: &[u8], signature: &[u8], public_key: &[u8]) -> bool {
    ...
    let group = EcGroup::from_curve_name(Nid::SECP384R1).unwrap();
    let p384_pub = EcPoint::from_bytes(group.as_ref(), &public_key[1..P384_PUBLIC_KEY_SIZE], ctx);
    if p384_pub.is_err() {
        return false;
    }
